...
char *vmsfs_describe_error()
{
  ...
  switch(vmsfs_errno)
  {
   ...
   case VMSFS_EOUS:
     ...
     strcat(buf, itoa_local(ous_want));
     strcat(buf, " blocks.");
     return buf;
   case VMSFS_EFORMAT:
   ...
   default:
     ...
     strcat(buf, itoa_local(vmsfs_errno));
     strcat(buf, "'!");
     return buf;
  }
}
...
int vmsfs_check_unit(int unit, int part, struct vmsinfo *info)
{
  ...
  //FIXME: Timer problem if docmd was done somewhere else within 15000�s?
